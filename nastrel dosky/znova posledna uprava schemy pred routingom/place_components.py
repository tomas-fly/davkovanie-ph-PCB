...
PLACEMENT["J1"]   = (BX + 10, BY + 8,  0)     # AC in 16×11mm
PLACEMENT["F1"]   = (BX + 30, BY + 8,  0)     # Fuse 13×13mm

PLACEMENT["U1"]   = (BX + 9,  BY + 42, 0)     # origin at left! local=(-4.5,-21.8)-(55.5,14.2)

PLACEMENT["C1"]   = (BX + 10, BY + 68, 0)     # PSU bypass below HLK
PLACEMENT["D1"]   = (BX + 18, BY + 68, 0)     # LED
PLACEMENT["R1"]   = (BX + 26, BY + 68, 0)     # LED R
...
# Buck 12V→5V
PLACEMENT["C2"]   = (PX - 2,  BY + 10, 0)     # 12mm electro cap
PLACEMENT["U2"]   = (PX + 18, BY + 10, 0)     # TO-263 17×11mm
...
PLACEMENT["C6"]   = (PX - 2,  BY + 54, 0)
PLACEMENT["C7"]   = (PX + 8,  BY + 54, 0)
PLACEMENT["U4"]   = (PX + 24, BY + 52, 0)     # D2PAK 17×11mm
...
MX = BX + 68  # MOSFET zone starts at x=68
PLACEMENT["Q1"]   = (MX + 2,  BY + 114, 0)    # TO-220 10×15mm
PLACEMENT["R20"]  = (MX - 10, BY + 114, 0)
PLACEMENT["R24"]  = (MX - 10, BY + 122, 0)
...
# --- ANALOG FRONT-END (x=140..215) ---
AX = BX + 135

# pH section — BNC horizontal at right edge, rotation 270° (body protrudes right)
# At 270°, BNC courtyard: x=-7.5..+17.4, y=-7.5..+7.5 from center
# Place at board edge: center x = BX+BW-8 so pads stay inside
PLACEMENT["P1"]   = (BX + BW - 8, BY + 24, 270)   # pH BNC, right edge
...
# ORP section (shifted down 44mm for clearance)
DY = 44
PLACEMENT["P2"]   = (BX + BW - 8, BY + 24 + DY, 270)  # ORP BNC, right edge
PLACEMENT["R9"]   = (AX + 48, BY + 12 + DY, 0)
PLACEMENT["C19"]  = (AX + 48, BY + 20 + DY, 0)
...
# --- CONNECTORS (top/bottom edges, right side clear for BNC) ---
PLACEMENT["J2"]   = (AX + 30, BY + 4, 90)     # Display 1×14 — top edge, rotated
PLACEMENT["J7"]   = (AX + 30, BY + BH - 4, 90) # GPIO 2×10 — bottom edge, rotated
PLACEMENT["J8"]   = (AX + 56, BY + BH - 4, 0)  # UART 1×4 — bottom edge

PLACEMENT["J3"]   = (AX + 8,  BY + 108, 0)    # 1-Wire JSTs (14mm spacing)
PLACEMENT["J4"]   = (AX + 22, BY + 108, 0)
PLACEMENT["J5"]   = (AX + 36, BY + 108, 0)
PLACEMENT["J6"]   = (AX + 50, BY + 108, 0)
PLACEMENT["R31"]  = (AX + 8,  BY + 100, 0)
    ...
    for i in range(len(refs)):
        for j in range(i + 1, len(refs)):
            ...
            if boxes_overlap(b1, b2):
                overlaps_before += 1
    # Debug: show actual bboxes for problematic components
    for dbg in ["U1", "C4", "C6", "C7"]:
        if dbg in local_bboxes:
            lb = local_bboxes[dbg]
            pos = PLACEMENT[dbg]
            ab = get_abs_bbox(dbg, pos, local_bboxes)
            print(f"  {dbg}: local=({lb[0]:.1f},{lb[1]:.1f})-({lb[2]:.1f},{lb[3]:.1f}) pos=({pos[0]:.0f},{pos[1]:.0f}) abs=({ab[0]:.1f},{ab[1]:.1f})-({ab[2]:.1f},{ab[3]:.1f})")
